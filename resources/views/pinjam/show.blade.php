            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="mt-3">
                        ...
                        @if($pinjam->status == 'pending')
                            @if(Auth::id() == $pinjam->user_id)
                                <form action="/pinjam/{{ $pinjam->id }}" method="POST" class="d-inline">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit" class="btn btn-danger" onclick="return confirm('Yakin ingin membatalkan peminjaman?')">
                                        Batalkan Peminjaman
                                    </button>
                                </form>
                            @endif
                        ...
                        @if($pinjam->status == 'approved' && Auth::user()->is_admin)
                            <form action="/pinjam/{{ $pinjam->id }}/return" method="POST" class="d-inline">
                                @csrf
                                <button type="submit" class="btn btn-info" onclick="return confirm('Konfirmasi pengembalian barang?')">
